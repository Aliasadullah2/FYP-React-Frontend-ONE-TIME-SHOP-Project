...
import { Container,Col, Row, Card, CardBody, CardText, Button, Input } from 'reactstrap'
...
// }
  ...
  return (
    ...
    <Container className='bg-primary' >
      <Link  className='btn btn-secondary mb-2 mt-2' to="/mentor/buyer/BuyerDropshipping">Back</Link> {product && (<Link to='' className='text-light'>/{product.prodName}</Link>)}
      <Row >
        <Col md={{size:12}}>
        <Card >
          {
            (product) && (
              <CardBody className=''>
            <CardText>
              Product By <b>{product.user.name}</b> on <b>{printDate(product.addedDate)}</b>
