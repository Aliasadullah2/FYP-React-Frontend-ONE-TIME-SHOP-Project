...
function MyProduct({product={ id:-1, prodName:"Product Name",prodDesc:"Product Despcription",prodPrice:"Product Price" } } ) {
  ...
  <CardBody  >
   ...
   <div style={{display:'flex',flexDirection:'row',justifyContent:'center'}} > Price &nbsp;<h6> {product.prodPrice.substring(0,30)}</h6></div>
    ...
    <Link className="btn btn-primary" to={`/mentor/MyProductPage/`+product.prodId} >ReadMore</Link>
            ...
            {
                isLoggedIn && (user && user.id === product.user.id ? <Button tag={Link} to={`/mentor/updatepage/${product.prodId}`} color='warning' className='ms-3'  >Update</Button> : '')
            }
